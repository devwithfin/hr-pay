...
const { Attendance, OvertimeRequest, PayrollDetail, PayrollPeriod} = db;
...
export const getSummaryData = async (req: Request, res: Response) => {
    try {
      ...
      const attendanceChart = attendanceRaw.map((a:any) => ({
      ...
      const parseTimeToMinutes = (timeStr:any) => {
        if (!timeStr) return 0;
        const [hh, mm, ss] = timeStr.split(":").map(Number);
        return hh * 60 + mm + (ss || 0) / 60;
      };

      const overtimeChart = overtimeRecords.map((ot:any) => {
