...
const {Employee, Department, Position, Attendance, OvertimeRequest, PayrollPeriod} = db;
...
export const getSummaryData = async (req: Request, res: Response) => {
    try {
      ...
      const overtimeTable = overtimeHistory.map((o:any) => ({
      ...
      const employeeTable = employeeList.map((e:any) => ({
