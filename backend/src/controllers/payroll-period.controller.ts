...
function calculatePph21(grossIncome:any, ptkp:any, brackets:any) {
...
export const createDraftPayroll = async (req: Request, res: Response) => {
    ...
    try {
      ...
      for (const emp of employees) {
        ...
        const totalAllowances = allowances.reduce(
          (sum:any, a:any) => sum + parseFloat(a.amount || 0),
          0
        );
        ...
        const totalOvertimePay = overtimes.reduce(
          (sum:any, o:any) => sum + parseFloat(o.overtime_amount || 0),
          0
        );
        ...
        const totalDeductions = deductions.reduce(
          (sum:any, d:any) => sum + parseFloat(d.amount || 0),
          0
        );
...
export const createFinalPayroll = async (req: Request, res: Response) => {
    const periodId = req.params.id;
    try {
      const period = await PayrollPeriod.findByPk(periodId);
      if (!period) return res.status(404).json({ message: "Period not found" });
      ...
      for (const item of payrolls) {
        ...
        const totalFixedAllowances = fixedAllowances
          .filter((a:any) => a.allowance?.is_fixed === true)
          .reduce((sum:any, a:any) => sum + parseFloat(a.amount || 0), 0);
...
export const createPayrollTransfer = async (req: Request, res: Response) => {
    const periodId = req.params.id;
    try {
      const period = await PayrollPeriod.findByPk(periodId);
      if (!period)
